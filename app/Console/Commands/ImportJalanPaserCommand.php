...
class ImportJalanPaserCommand extends Command
{
    protected $signature = 'import:jalan-paser';
    protected $description = 'Import Paser Jalan GeoJSON data';

    public function handle()
    {
        $this->info('Starting Paser Jalan data import...');

        // Clear existing records
        LN_Jalan_Paser::truncate();

        // Path to the GeoJSON file
        $geoJsonPath = public_path('assets/Jalan/jalan-kabupaten/Paser.json');
        ...
        foreach ($features as $feature) {
            ...
            $model = new LN_Jalan_Paser();
            $model->OBJECTID_1 = $properties['OBJECTID_1'] ?? null;
            $model->OBJECTID_2 = $properties['OBJECTID_2'] ?? null;
            ...
            $model->pnj = $properties['pnj'] ?? null;
            $model->Id_Paser = $properties['Id'] ?? null;
            $model->Shape_Leng = $properties['Shape_Leng'] ?? null;
            $model->X_Ak = $properties['X_Ak'] ?? null;
            $model->Y_Ak = $properties['Y_Ak'] ?? null;
            $model->X_Aw = $properties['X_Aw'] ?? null;
            $model->Y_Aw = $properties['Y_Aw'] ?? null;
            $model->No = $properties['No'] ?? null;
            $model->geom = $geometry;
        ...
        $this->info("Successfully imported {$importedCount} Paser Jalan records.");
        return 0;
    }
}