...
class ImportJaringanListrikBalikpapanCommand extends Command
{
    protected $signature = 'jaringan:bpn-import 
                            {--file= : Path relatif dari folder public ke file GeoJSON}';

    protected $description = 'Import data jaringan listrik Balikpapan dari file GeoJSON ke tabel table__jaringan__listrik__balikpapan';
    ...
    public function handle(): int
    {
        ...
        if ($fileOpt === '') {
            $this->error('Option --file wajib diisi, contoh: --file="public\assets\Jaringan-Listrik\LN_Jaringan_Listrik_Balikpapan.json"');
            return 1;
        }
        ...
        $this->info('🚧 Memulai import data jaringan listrik Balikpapan ...');
        ...
        foreach ($data['features'] as $index => $feature) {
            ...
            // Mapping dari properties GeoJSON
            ...
            // Pakai OBJECTID kalau ada, sebagai key unik
            if ($objectId !== null) {
                $model = JaringanListrikBalikpapan::firstOrNew(['objectid' => $objectId]);
            } else {
                $model = new JaringanListrikBalikpapan();
            }
