...
class ImportSistemJaringanEnergiKukarCommand extends Command
{
    protected $signature = 'sistem-energi:kukar-import 
                            {--file= : Path relatif dari folder public ke file GeoJSON}';

    protected $description = 'Import data sistem jaringan energi Kutai Kartanegara dari file GeoJSON ke tabel table__l_n__sistem__jaringan__energi__kukar';
    ...
    public function handle(): int
    {
        ...
        if ($fileOpt === '') {
            $this->error('Option --file wajib diisi, contoh: --file="public\assets\Jaringan-Energi\LN_Sistem_Jaringan_Energi_Kukar.json"');
            return 1;
        }
        ...
        $this->info('🚧 Memulai import data Sistem Jaringan Energi Kukar ...');
        ...
        foreach ($data['features'] as $index => $feature) {
            ...
            if ($objectId !== null) {
                $model = LN_Sistem_Jaringan_Energi_Kukar::firstOrNew(['objectid' => $objectId]);
            } else {
                $model = new LN_Sistem_Jaringan_Energi_Kukar();
            }
