...
class ImportLn2SutmPaserCommand extends Command
{
    protected $signature = 'sutm2:paser-import 
                            {--file= : Path relatif dari folder public ke file GeoJSON}';

    protected $description = 'Import data LN2 SUTM Paser dari file GeoJSON ke tabel table__l_n2__sutm__paser';
    ...
    public function handle(): int
    {
        ...
        if ($fileOpt === '') {
            $this->error('Option --file wajib diisi, contoh: --file="public/assets/Jaringan-Listrik/LN2_SUTM_Paser.json"');
            return 1;
        }
        ...
        $this->info('Memulai import data LN2 SUTM Paser ...');
        ...
        foreach ($data['features'] as $index => $feature) {
            ...
            $shapeLeng  = $props['shape_Leng']  ?? $props['SHAPE_Leng'] ?? null;
            ...
            if ($objectId !== null) {
                $model = LN2_SUTM_Paser::firstOrNew(['objectid' => $objectId]);
            } else {
                $model = new LN2_SUTM_Paser();
            }
