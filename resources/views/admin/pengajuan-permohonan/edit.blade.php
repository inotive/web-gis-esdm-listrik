...
@section('content')
    ...
    <section class="card">
        <div class="status-info">
            <strong>Perhatian:</strong> Anda hanya dapat mengedit permohonan dengan status <strong>Pending</strong>.
        </div>

        <form method="POST" action="{{ route('admin.pengajuan-permohonan.update', $permohonanUser) }}" id="permohonanForm">
            @csrf
            @method('PUT')
                        ...
                        @switch($question->tipe)
                            ...
                            @case('file')
                                <input type="file" name="jawaban[{{ $questionId }}]" class="input"
                                    {{ $question->wajib && !$answer ? 'required' : '' }} />
                                @if ($answer)
                            ...
                            @case('file_multiple')
                                <input type="file" name="jawaban[{{ $questionId }}][]" class="input"
                                    {{ $question->wajib && !$answer ? 'required' : '' }} multiple />
                                @if ($answer)
                                    <div style="margin-top: 8px; font-size: 12px; color: #64748B;">
                                        File saat ini: {{ is_array($answer) ? implode(', ', $answer) : $answer }}
                                    </div>
                                @endif
                            @break
            ...
            <div class="form-group">
                <label class="label" for="keterangan">Keterangan Tambahan</label>
                <textarea name="keterangan" id="keterangan" class="input" rows="4"
                    placeholder="Tambahkan keterangan jika diperlukan...">{{ old('keterangan', $permohonanUser->keterangan) }}</textarea>
                @error('keterangan')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
                ...
                Swal.fire({
                    ...
                    html: `
                        <div style="text-align: left; padding: 10px;">
                            <p style="margin-bottom: 12px;">Apakah Anda yakin ingin menyimpan perubahan permohonan ini?</p>
                            <div style="background: #F3F4F6; padding: 12px; border-radius: 8px; font-size: 13px;">
                                <strong>Jenis Permohonan:</strong><br>
                                {{ $permohonanUser->permohonan->nama }}
                            </div>
                            <p style="margin-top: 12px; font-size: 12px; color: #64748B;">
                                <i class="ri-information-line"></i>
                                Perubahan akan disimpan dan permohonan tetap berstatus Pending.
                            </p>
                        </div>
                    `,
                    icon: 'question',
                    ...
                    confirmButtonText: '<i class="ri-check-line"></i> Ya, Simpan!',
