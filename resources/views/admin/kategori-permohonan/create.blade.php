...
<div class="page-head">
  <div>
    ...
    <div class="page-title">Tambah Permohonan</div>
  </div>
</div>
...
<section class="card">
  <form method="POST" action="{{ route('admin.kategori-permohonan.store') }}" id="formPermohonan">
    ...
    <div class="form-group">
      ...
      <input type="text" name="nama" class="input" value="{{ old('nama') }}" placeholder="Masukkan nama permohonan" required>
    </div>

    <div class="form-group">
      <label class="label">Jenis Permohonan <span style="color:#DC2626">*</span></label>
      <select name="jenis_permohonan" class="input" required>
        <option value="" disabled {{ old('jenis_permohonan') ? '' : 'selected' }}>Pilih jenis permohonan</option>
        <option value="desa" {{ old('jenis_permohonan') == 'desa' ? 'selected' : '' }}>Desa</option>
        <option value="perusahan" {{ old('jenis_permohonan') == 'perusahan' ? 'selected' : '' }}>Perusahaan</option>
      </select>
    </div>

    <div class="form-group">
      <label class="label">Keterangan</label>
      <textarea name="keterangan" class="input" rows="3" placeholder="Masukkan keterangan">{{ old('keterangan') }}</textarea>
    </div>
    ...
    <div style="margin-top: 24px; display: flex; gap: 10px;">
      ...
      <button type="submit" class="btn btn-primary">
        <i class="ri-save-3-line"></i> Simpan Permohonan
      </button>
    </div>
  </form>
</section>
...
function updateQuestionNumbers() {
  ...
  questions.forEach((q, idx) => {
    ...
    // Update urutan input
    const urutanInput = q.querySelector('input[name*="[urutan]"]');
    if (urutanInput) {
...
function removeOption(questionIndex, optionIndex) {
  const optionItem = document.querySelector(`#options_list_${questionIndex} [data-option-index="${optionIndex}"]`);
  if (optionItem) {
    optionItem.remove();
...
function reindexOptions(questionIndex) {
  ...
  options.forEach((opt, idx) => {
    ...
    inputs.forEach(input => {
      ...
      if (input.id) {
        input.id = input.id.replace(/opt_wajib_\d+_\d+/, `opt_wajib_${questionIndex}_${idx}`);
      }
    });
  });
}
