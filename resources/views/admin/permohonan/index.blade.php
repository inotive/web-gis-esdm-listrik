...
@section('title', 'Dashboard ESDM - Manajemen Permohonan')
...
@section('content')
  ...
  <section class="card" style="margin-top:18px;">
    <div class="card-header">
      <form id="filterForm" class="toolbar" method="GET" action="{{ route('admin.permohonan.index') }}">
        <div class="input-group w-search">
          <span class="input-group-text"><i class="ri-search-line"></i></span>
          <input type="text" name="q" value="{{ request('q') }}" class="form-control"
                 placeholder="Cari Nama atau Jenis Permohonan" autocomplete="off">
        </div>
      </form>
    </div>
          ...
          <tbody>
            @forelse ($permohonans as $i => $permohonan)
              <tr>
                ...
                <td class="col-aksi">
                  <a href="{{ route('admin.permohonan.edit', $permohonan) }}" class="btn-ico edit" title="Edit">
                  ...
                  <form action="{{ route('admin.permohonan.destroy', $permohonan) }}" method="POST" style="display:inline-block;margin:0;" class="form-delete-permohonan" data-name="{{ $permohonan->nama }}">
                    @csrf @method('DELETE')
