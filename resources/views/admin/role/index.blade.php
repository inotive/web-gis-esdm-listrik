        ...
        <div class="card-header">
            ...
            <form id="filterForm" class="toolbar" method="GET" action="#" onsubmit="return false;">
                <div class="input-group w-search">
                    ...
                    <input type="search" id="roleSearch" value="{{ request('q') }}" class="form-control"
                        placeholder="Cari Nama Role..." autocomplete="off" spellcheck="false">
                    @if (request('q'))
                        <button type="button" class="btn-ghost" id="btnClearSearch" title="Bersihkan"><i
                                class="ri-close-line"></i><span class="d-none d-sm-inline"> Clear</span></button>
                    @endif
                </div>
                        ...
                        @foreach ($data as $value)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <span class="badge-hak badge-light-primary-hak fs-7 fw-bold">{{ $permCount }}</span>
                                </td>
                                <td class="col-aksi">
                                    @can('role.permission')
                                        <a href="{{ route('admin.hak-akses.role.permissions', $value->id) }}" class="btn-ico"
                                            title="Kelola Hak Akses" data-bs-toggle="tooltip" data-bs-placement="top">
                                            <i class="fas fa-key fa-icon"></i>
                                        </a>
                                    @endcan

                                    @can('role.edit')
                                        <button class="btn-ico edit" title="Edit Role"
                                            data-modal-target="#kt_modal_{{ $value->id }}">
                                    ...
                                    @can('role.delete')
                                        <button data-route="{{ route('admin.hak-akses.role.destroy', $value->id) }}"
                                            class="btn-ico delete" title="Hapus" onclick="destroyItem(this)">
                    ...
                    <div class="show-wrap">
                        ...
                        <select class="form-select" id="perPageSelect" aria-label="Jumlah baris per halaman">
                            @foreach ([10, 25, 50, 100] as $pp)
                                <option value="{{ $pp }}"
                                    {{ (int) request('per_page', 10) === $pp ? 'selected' : '' }}>{{ $pp }}
                                </option>
                            @endforeach
                        </select>
                        <span>per page</span>
    ...
    <script>
        jQuery(function($) {
            ...
            document.querySelectorAll(
                    '.modal [data-bs-dismiss="modal"], .modal [data-dismiss="modal"], .modal [data-close]')
