...
@section('title', 'Edit Data Asset - Dinas ESDM')

@section('page-title', 'Edit Data Asset')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title align-items-start flex-column mb-0">
                <span class="card-label fw-bold fs-3">Form Edit Data Asset</span>
                <span class="text-muted mt-1 fw-semibold fs-7">{{ $asset->kode_asset }} - {{ $asset->nama_asset }}</span>
            </h3>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('admin.asset.update', $asset) }}" method="POST" enctype="multipart/form-data" data-region-form="asset">
                @csrf
                @method('PUT')
