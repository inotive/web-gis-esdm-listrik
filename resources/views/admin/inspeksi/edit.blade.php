...
@section('title', 'Dashboard ESDM - Edit Inspeksi')
...
@section('content')
    <div class="page-head">
        <div>
            ...
            <div class="page-title">Edit Inspeksi</div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.inspeksi.update', $inspeksi) }}" enctype="multipart/form-data">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal" class="form-control"
                        value="{{ old('tanggal', $inspeksi->tanggal->format('Y-m-d')) }}" required>
                    @error('tanggal')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="referensi_izin" class="form-control" placeholder="Contoh: No. 123/ESDM/2024"
                        value="{{ old('referensi_izin', $inspeksi->referensi_izin) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="perusahaan_id" class="form-control select2" required>
                        ...
                        @foreach ($perusahaans as $p)
                            <option value="{{ $p->id }}" @selected(old('perusahaan_id', $inspeksi->perusahaan_id) == $p->id)>
                                {{ $p->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" value="{{ $inspeksi->pengguna->name ?? '-' }}" disabled
                ...
                <div class="form-group">
                    ...
                    @if ($inspeksi->lampiran)
                        <div class="current-file">
                            ...
                            File saat ini:
                            <a href="{{ Storage::url($inspeksi->lampiran) }}" target="_blank">Lihat File</a>
                        </div>
                    @endif
                    <div class="file-input-wrapper">
                        <label for="lampiran" class="file-input-label">
                            <i class="ri-attachment-2"></i>
                            <span>{{ $inspeksi->lampiran ? 'Upload file baru untuk mengganti' : 'Upload Lampiran' }} (PDF,
                                JPG, PNG, DOC - Max 5MB)</span>
                        </label>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea name="berita_acara" class="form-control" rows="6"
                        placeholder="Masukkan isi berita acara inspeksi (opsional)">{{ old('berita_acara', $inspeksi->berita_acara) }}</textarea>
                    @error('berita_acara')
                        <div class="form-helper" style="color: #ef4444;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea name="catatan" class="form-control" placeholder="Masukkan catatan inspeksi (opsional)">{{ old('catatan', $inspeksi->catatan) }}</textarea>
                    @error('catatan')
                        <div class="form-helper" style="color: #ef4444;">{{ $message }}</div>
                    @enderror
                </div>
